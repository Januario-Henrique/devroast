 ...
 */
function analyzeCodeCharacteristics(
	_code: string,
	language: string,
	lines: string[],
) {
...
function analyzeJSCode(
	...
	if (code.includes("eval(")) {
		issues.push({
			type: "error",
			message: "Avoid using eval() - it's dangerous and inefficient",
...
function analyzeCPPCode(
	...
	if (code.includes("gets(") || code.includes("strcpy(")) {
		issues.push({
			type: "error",
			message:
				"Use safer alternatives to gets()/strcpy() to prevent buffer overflows",
			line:
				findLineNumber(code, "gets(") !== -1
					? code.indexOf("gets(")
					: code.indexOf("strcpy("),
		});
	}
	...
	if (code.includes("malloc(") && !code.includes("free(")) {
		issues.push({
			type: "warning",
			message: "Remember to free() memory allocated with malloc()",
...
function analyzeGoCode(
	...
	if (code.includes("if err != nil") && !code.includes("return")) {
		issues.push({
			type: "warning",
			message: "Handle errors properly instead of just checking them",
			line: findLineNumber(code, "if err != nil"),
		});
	}
...
function analyzeRustCode(
	...
	if (code.includes("unwrap()") && !code.includes("expect(")) {
		issues.push({
			type: "warning",
			message: "Consider using expect() with a message instead of unwrap()",
			line: findLineNumber(code, "unwrap()"),
		});
	}
...
function generateSuggestions(analysis: any, language: string): string[] {
	...
	if (analysis.warnings > 3) {
		suggestions.push(
			"Address warnings to improve code quality and prevent future bugs",
		);
	}
 ...
 */
function generateSuggestedLines(
	originalCode: string,
	analysis: any,
	_language: string,
): string[] {

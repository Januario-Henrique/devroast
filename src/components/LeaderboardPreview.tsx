		...
		<div className="max-w-container-lg w-full">
			...
			<div className="border border-border-primary rounded-lg overflow-hidden">
				<div className="h-10 bg-bg-surface border-b border-border-primary flex items-center px-5">
					...
					<span className="w-24 text-text-tertiary text-xs font-medium">
						lang
					</span>
				</div>
				...
				{mockLeaderboard.map((entry, idx) => (
					<div
						...
						className={`flex items-center px-5 py-4 ${
							idx !== mockLeaderboard.length - 1
								? "border-b border-border-primary"
								: ""
						}`}
					>
						<span
							className={`w-12 text-xs ${entry.rank === 1 ? "text-accent-amber" : "text-text-secondary"}`}
						>
							{entry.rank}
						</span>
						<span className="w-16 text-accent-red text-xs font-bold">
							{entry.score}
						</span>
						<div className="flex-1 flex flex-col gap-0.5">
							{entry.code.map((line, i) => (
								<span
									key={i}
									className="text-text-primary text-xs font-mono truncate max-w-md"
								>
									{line}
								</span>
							))}

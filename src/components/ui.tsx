...
export function ToggleSwitch(props: {
	...
	return React.createElement(
		...
		React.createElement(
			...
			{
				className:
					"relative inline-flex h-[24px] w-[48px] cursor-pointer items-center",
			},
			...
			React.createElement("span", {
				className:
					"pointer-events-none absolute left-[2px] top-[2px] h-[20px] w-[20px] rounded-full bg-white transition-all duration-200 peer-checked:left-[26] peer-checked:border-0 peer-checked:bg-black",
			}),
		),
		React.createElement(
			"span",
			{
				className: `text-sm font-medium ${isChecked ? "text-accent-green" : "text-text-secondary"}`,
			},
...
export function Badge(props: {
	...
	const variantClasses: Record<string, string> = {
		...
		verdict:
			"bg-gradient-to-r from-accent-green via-accent-amber to-accent-red text-white text-xs font-bold px-4 py-2",
	};
	...
	return React.createElement(
		"span",
		{ className: `${baseClasses} ${variantClass} ${props.className || ""}` },
		props.children,
	);
}

export function ScoreRing(props: { score: number; className?: string }) {
	return React.createElement(
		"div",
		{
			className: `relative w-[160px] h-[160px] flex items-center justify-center ${props.className || ""}`,
		},
		React.createElement("div", {
			className: "absolute inset-0 rounded-full border-8 border-border-primary",
		}),
		React.createElement("div", {
		...
		React.createElement(
			...
			React.createElement(
				"span",
				{ className: "text-accent-amber text-[48px] font-bold" },
...
export function CodeBlock(props: {
	...
	const lineNumbers = Array.from(
		{ length: Math.max(props.code.length, 1) },
		(_, i) => i + 1,
	);
	...
	return React.createElement(
		...
		{
			className:
				"border border-border-primary rounded-lg overflow-hidden shadow-sm",
		},
		React.createElement(
			"div",
			{
				className:
					"h-12 bg-bg-surface border-b border-border-primary flex items-center px-4",
			},
			props.showLineNumbers &&
				React.createElement(
					...
					{
						className:
							"w-12 bg-bg-surface border-r border-border-primary p-4 pt-4 flex flex-col gap-2 items-end",
					},
			...
			React.createElement(
				...
				props.language &&
					React.createElement(
						"span",
						{ className: "mb-2 block text-text-tertiary text-xs font-mono" },
						props.language,
					),
				React.createElement(
					"pre",
					{
						className:
							"text-text-primary text-xs font-mono whitespace-pre-wrap",
					},
					props.code.join("\n"),
				),
			),
		...
		props.original.map((line, index) =>
			React.createElement(
				"div",
				{
					key: `orig-${index}`,
					className:
						"flex items-center gap-2 bg-red-500/5 border-l-4 border-red-500",
				},
				React.createElement(
					"span",
					{ className: "w-5 text-xs font-mono text-accent-red" },
					"-",
				),
				React.createElement(
					"span",
		...
		props.modified.map((line, index) =>
			React.createElement(
				"div",
				{
					key: `mod-${index}`,
					className:
						"flex items-center gap-2 bg-green-500/5 border-l-4 border-green-500",
				},
				React.createElement(
					"span",
					{ className: "w-5 text-xs font-mono text-accent-green" },
					"+",
				),
				React.createElement(
					"span",
...
export function TableRow(props: {
	...
	return React.createElement(
		...
		{
			className:
				"border-b border-border-primary last:border-b-0 flex items-center px-6 py-5",
		},
		React.createElement(
			"div",
		...
		React.createElement(
			...
			{
				className:
					"w-20 text-accent-red text-xs font-bold flex items-center justify-center",
			},
			props.score.toString(),
		),
		React.createElement(
			...
			{
				className:
					"flex-1 text-text-secondary text-xs font-mono truncate max-w-md",
			},
			props.codePreview,
		),
		React.createElement(
			"div",
			{ className: "w-20 text-text-secondary text-xs" },
			props.language,
		),
		React.createElement(
			"div",
			{ className: "w-16 text-text-tertiary text-xs" },
			`${props.lines} lines`,
		),
	);
}
...
export function SectionTitle({ children, className }: SectionProps) {
	return (
		<span className={`text-accent-green text-sm font-bold ${className || ""}`}>
			{children}
		</span>
	);
}
...
export function CodeWindow({ children, className }: CodeWindowProps) {
	return (
		<div
			className={`border border-border-primary rounded-lg overflow-hidden ${className || ""}`}
		>
			{children}
		</div>
	);
...
export function CodeWindowHeader({
	...
	return (
		<div
			className={`h-10 bg-bg-surface border-b border-border-primary flex items-center px-4 ${className || ""}`}
		>
...
export function CodeWindowContent({
	children,
	code,
	className,
}: {
...
export function IssueCard({ children, className }: IssueCardProps) {
	return (
		<div
			className={`border border-border-primary rounded-lg p-5 flex flex-col gap-3 ${className || ""}`}
		>
			{children}
		</div>
	);
...
export function IssueCardSeverity({
	severity,
	className,
}: {
...
export function IssueCardTitle({ children, className }: SectionProps) {
	return (
		<h3 className={`text-text-primary text-sm font-medium ${className || ""}`}>
			{children}
		</h3>
	);
}
...
export function IssueCardDescription({ children, className }: SectionProps) {
	return (
		<p
			className={`text-text-secondary text-xs leading-relaxed ${className || ""}`}
		>
			{children}
		</p>
	);
